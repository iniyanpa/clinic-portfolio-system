  ...
  return (
    <div className="space-y-8 animate-in fade-in">
      <div className="flex justify-between items-end">
        <div>
          <h2 className="text-4xl font-heading font-bold text-primary">OPD Schedule</h2>
          <p className="subheading text-secondary font-bold text-[10px] tracking-widest uppercase">Daily Clinic Calendar</p>
        </div>
            ...
            <div key={appt.id} className="p-8 bg-white border border-slate-200 rounded-[2.5rem] flex flex-col md:flex-row md:items-center justify-between gap-6 hover:shadow-lg transition-all">
              ...
              <div className="flex items-center gap-6">
                ...
                {appt.status === 'Scheduled' && (
                  <button onClick={() => setCheckingInApptId(appt.id)} className="bg-secondary text-white px-8 py-3 rounded-2xl text-xs font-bold hover:bg-primary transition-all shadow-md">Start Triage</button>
                )}
              </div>
            </div>
          );
        })}
        {appointments.length === 0 && <div className="p-20 text-center text-slate-300 italic font-bold">No clinical appointments scheduled.</div>}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-primary/20 backdrop-blur-md p-4">
          <div className="bg-white w-full max-w-lg rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in">
            <div className="bg-primary p-10 text-white text-center">
              <h3 className="text-2xl font-bold font-heading">Book Appointment</h3>
            </div>
            <form onSubmit={handleBooking} className="p-10 space-y-6">
              <select required className="w-full bg-slate-50 p-4 rounded-2xl border border-slate-200 outline-none font-bold text-lg" value={newAppt.patientId} onChange={e => setNewAppt({...newAppt, patientId: e.target.value})}>
                <option value="">Registered Patient...</option>
              ...
              <div className="flex gap-4 pt-6">
                ...
                <button type="submit" className="flex-1 py-4 bg-primary text-white rounded-[2rem] font-bold shadow-xl hover:bg-secondary transition-all">Confirm</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-primary/20 backdrop-blur-md p-4">
          <div className="bg-white w-full max-w-md rounded-[3rem] shadow-2xl overflow-hidden">
             <div className="bg-secondary p-8 text-white text-center">
                <h3 className="text-2xl font-bold font-heading">Capture Vitals</h3>
             </div>
             <div className="p-10 space-y-6">
                <div className="grid grid-cols-2 gap-4">
                   <input className="bg-slate-50 p-4 rounded-xl font-bold outline-none border border-slate-200" placeholder="BP (120/80)" value={checkInVitals.bp} onChange={e => setCheckInVitals({...checkInVitals, bp: e.target.value})} />
                   ...
                   <input className="bg-slate-50 p-4 rounded-xl font-bold outline-none border border-slate-200" placeholder="Pulse" value={checkInVitals.pulse} onChange={e => setCheckInVitals({...checkInVitals, pulse: e.target.value})} />
                ...
                <textarea rows={3} className="w-full bg-slate-50 p-4 rounded-2xl font-bold outline-none border border-slate-200" placeholder="Complaints..." value={checkInVitals.symptoms} onChange={e => setCheckInVitals({...checkInVitals, symptoms: e.target.value})} />
                <div className="flex gap-4 pt-4">
                   <button onClick={() => setCheckingInApptId(null)} className="flex-1 py-4 text-slate-400 font-bold uppercase tracking-widest text-xs">Cancel</button>
                   <button onClick={handleCheckIn} className="flex-1 py-4 bg-primary text-white rounded-2xl font-bold shadow-xl hover:bg-secondary transition-all">Proceed</button>
                </div>
             </div>
          </div>
        </div>
